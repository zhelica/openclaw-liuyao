...
/** 年上起月口诀表（年干 -> 寅月起首天干索引） */
const YUE_JIA_KOUJUE: Record<string, number> = {
    '甲': 2, '己': 2,  // 甲己之年丙作首
    '乙': 4, '庚': 4,  // 乙庚之岁戊为头
    '丙': 6, '辛': 6,  // 丙辛之岁寻庚起
    '丁': 8, '壬': 8,  // 丁壬壬位顺行流
...
/** 季节调候用神 */
const SEASON_TIAOHOU: Record<string, string> = {
    '春': '金',  // 春天喜金
    '夏': '水',  // 夏天喜水
    '秋': '木',  // 秋天喜木
    '冬': '火'   // 冬天喜火
...
export function analyzeBazi(
    ...
    const diZhiGen = allZhi.filter(z =>
        ZHI_WUXING[z] === riWuXing || WUXING_SHENG[riWuXing] === ZHI_WUXING[z]
    ).length;
    const deDi = diZhiGen > 0;
    const ganShi = allGan.filter(g =>
        GAN_WUXING[g] === riWuXing || WUXING_SHENG[riWuXing] === GAN_WUXING[g]
    ).length;
    const deShi = ganShi >= 2;

    let isStrong: boolean;
    let wangShuaiDesc: string;
    ...
    // ==============================================
    function getTiaoHouYongShen(riGan: string, monthZhi: string): string {
        ...
        switch (riGan) {
            ...
            case '庚':
            case '辛':
                if (isWinter) return '水';
                if (isSummer) return '水';
                if (isSpring) return '金';
                if (isAutumn) return '水';
                return '水';
    
            // 木：甲乙
            case '甲':
            case '乙':
                if (isWinter) return '火';
                if (isSummer) return '水';
                if (isSpring) return '金';
                if (isAutumn) return '水';
                return '水';
            ...
            case '丙':
            case '丁':
                if (isWinter) return '木';
                if (isSummer) return '水';
                if (isSpring) return '金';
                if (isAutumn) return '木';
                return '水';
            ...
            case '己':
                if (isWinter) return '火';
                if (isSummer) return '水';
                if (isSpring) return '火';
                if (isAutumn) return '金';
                return '火';
    ...
    // ================= 核心修复 =================
    // 喜神 = 生用神的五行（正确逻辑）
    const WUXING_SHENG_BY: Record<string, string> = {
        '金': '土', '水': '金', '木': '水', '火': '木', '土': '火'
    };
...
function getShengXiao(zhi: string): string {
    const shengXiaoMap: Record<string, string> = {
        '子': '鼠', '丑': '牛', '寅': '虎', '卯': '兔',
        '辰': '龙', '巳': '蛇', '午': '马', '未': '羊',
        '申': '猴', '酉': '鸡', '戌': '狗', '亥': '猪'
    };
 ...
 * 获取五行平衡描述
 */
function getWuXingBalance(count: Record<string, number>): string {
...
function generateSuggestions(
    ...
    // 幸运颜色
    const colorMap: Record<string, { lucky: string[]; avoid: string[] }> = {
 ...
 * 格式化八字分析输出
 */
export function formatBaziAnalysis(analysis: BaziAnalysis): string {
    const lines: string[] = [];
 ...
 * 计算天干索引（60甲子循环）
 */
function getYearGanIndex(year: number): number {
    return (year - 4) % 10;
}
 ...
 * 计算地支索引（60甲子循环）
 */
function getYearZhiIndex(year: number): number {
    return (year - 4) % 12;
}
 ...
 * 计算年柱
 */
export function getYearZhu(year: number): string {
 ...
 * 计算月柱（基于年干的口诀计算）
 */
export function getMonthZhu(year: number, month: number): string {
 ...
 * 计算农历生日的八字
 */
export function getBaziFromLunar(year: number, month: number, day: number, hour: number, isLeapMonth = false) {
...
export function parseDateTime(input: string) {
    let year: number | undefined, month = 1, day = 1, hour = 0;
    let valid = false;
    ...
    if (/^\d+$/.test(digitsOnly)) {
        ...
        if (digitsOnly.length >= 6) month = parseInt(digitsOnly.substring(4, 6));
        if (digitsOnly.length >= 8) day = parseInt(digitsOnly.substring(6, 8));
        if (digitsOnly.length >= 10) hour = parseInt(digitsOnly.substring(8, 10));
    }
 ...
 * 获取今日黄历信息
 */
export function getHuangLi(year: number, month: number, day: number) {
 ...
 * 格式化黄历输出为美观的字符串
 */
export function formatHuangLi(huangLi: ReturnType<typeof getHuangLi>): string {
...
/** 八卦二进制映射表 */
export const TRIGRAM_MAP: Record<string, string> = {
    '111': 'Qian',   // 乾
    '011': 'Dui',    // 兑
    '101': 'Li',     // 离
    '001': 'Zhen',   // 震
...
/** 64 卦索引表 (上卦+下卦 -> ID) */
export const HEXAGRAM_DB: Record<string, number> = {
...
/** 卦象核心含义 */
export const GUA_MEANINGS: Record<string, string> = {
 ...
 * 根据二进制数组 (初->上) 获取卦象信息
 */
export function getGuaInfo(binArray: number[]): GuaResult | null {
 ...
 * 验证输入格式
 */
export function validateInput(inputStr: string): { valid: boolean; error?: string } {
 ...
 * 主处理函数：解析输入并计算五卦
 */
export function calculateFiveGua(inputStr: string): DivinationSession {
    const parts = inputStr.trim().split(/\s+/);
    ...
    if (!bian) throw new Error("变卦计算失败");
 ...
 * 从用户输入中提取问题部分
 */
export function extractQuestion(input: string, numbers: string): string {
